...
g4 <- ggplot(data = filter(dat4a, `VE disease` == "90%"), aes(x = `Age.target`, y = value / vaccine_n_phase1 * 100, fill = `Age.target`, alpha = name)) +
...
ggsave("plots/fig4_FVP_VEdis90.png", plot = g4, height = 6, width = 14)

#######################################################################

icg <- "LIC"
g4_icg <- ggplot(data = filter(dat4a, income_group == icg, `VE disease` == "90%"), aes(x = `Age.target`, y = value / vaccine_n_phase1 * 100, fill = `Age.target`, alpha = name)) +
  geom_bar(stat = "identity") +
...
ggsave(paste0("plots/fig4_", icg, "_FVP_VEdis90.png"), plot = g4_icg, height = 6, width = 7)
...
icg <- "HIC"
g4b_icg <- ggplot() +
  geom_line(data = filter(dat4b, max_coverage == 0, income_group == icg, `VE disease` == "90%"),
            aes(x = t, y = value / target_pop * 1e6, linetype = factor(max_coverage)), col = "black") +
  geom_line(data = filter(dat4b, date > as.Date("2021-01-01"), max_coverage != 0, income_group == "LMIC"), aes(x = t, y = value / target_pop * 1e6, col = `Age.target`, linetype = factor(max_coverage))) +
...
g4b_icg
ggsave(paste0("plots/fig4b_", icg, "_VEdis90.png"), plot = g4b_icg, height = 6, width = 10)
